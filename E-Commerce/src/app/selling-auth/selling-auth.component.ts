...
import { matchPasswordValidator, passwordStrengthValidator } from '../custom-validator/custom_Validator';
...
export class SellingAuthComponent {
  form = new FormGroup({
    ...
    repassword: new FormControl('',[Validators.required,matchPasswordValidator('password')]),
  });
  ...
  signUp(data:seller){
    this.sellerService.createSeller(data).subscribe(res =>{console.log("Service Completed")});
